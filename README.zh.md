...
- **设置**：在 DeepSeek Harness 设置里新增「改动审批」页，分为「差异视图」（代码字号、行高、新增 / 删除颜色）、「快捷键」（重绑面板各类按键组合）与「浮窗覆盖」（面板自身弹出菜单里的那四个边的开关）三组，另有复制后自动粘贴、导入时是否包含未跟踪文件、diff 的 Tab 宽度（2 / 4 / 8 空格）、双栏对比开关、差异跳转行距、Markdown 预览默认与宽度，以及快速呼出快捷键。
