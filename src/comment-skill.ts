...
- **不要用 Markdown 的表格**（也别用列表、标题、围栏代码块这类块级语法）：讨论区是**按行**排版、行高按行算的，
  表格进来会读成一堆竖线和横线，还会把块高算错、把下面的代码顶开。要对比几项就写成一行一句，
  例如「A 走快路径；B 走慢路径」，需要多行对比就直接说"这条在批注里说不清，去下方输入框聊"。
  行内语法只有 \`代码\` 和 **加粗** 是被支持的。
