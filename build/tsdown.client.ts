/**
 * tsdown preset for this plugin's client bundle. Copied from deepseek-harness
 * `packages/client/tsdown.client.ts` (MIT) with the platform-module import
 * rebased to `./platform.ts`, so this plugin repo builds standalone.
 *
...
/**
 * The runtime snapshot-store engine lives in dsh-client-runtime pending its
 ...
 * plus the browser client bundle. Both halves emit during the Client pass by
 * default. A package-level tsdown.config.ts REPLACES the root workspace
 * layout, so the lib half must be restated here — dropping it leaves the
 * package without lib/index.js and the host Loader cannot import its node
 * half.
...
function clientConfig(id: string, entry: string): UserConfig {
  return {
    ...
    // the build's NODE_ENV so a dev build keeps the dev-branch semantics;
    // artifacts default to production.
    define: {
    ...
    outputOptions: {
      ...
      // The map is served from /plugins/<scoped-package>/client.js.map; local
      // sources stay relative to the bundle and resolve through the served
      // map.
